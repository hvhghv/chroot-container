...
3. `init.sh`不会加载`etc/chroot-init/_environment.sh`的环境变量,只有`entrypoint.sh`才会加载
4. 对于Android Chroot容器，目前仅考虑该容器随宿主系统持续运行，不会中途关闭停止，因此删除了`container-exit.sh`文件
5. 对于Android Chroot容器，其Android Linux内核执行unshare/nsenter时绝大多数情况是不支持--pid选项，因此会有所修改
## 运行
